...
export default function ManagerPrivacyDashboardPage() {
    ...
    const handleExportPDF = async () => {
        try {
            ...
            const metrics = [
                ['Sessions Created', c.sessions_created],
                ['Credentials Issued', c.credentials_issued],
                ['Credentials Revoked', c.credentials_revoked],
                ['Data Expired Naturally', c.data_expired],
                ['Data Conditionally Persisted', c.data_conditionally_persisted],
                ['Data Minimization Rate', `${c.minimization_rate_percent}%`],
            ];
            ...
            doc.setTextColor(107, 107, 107);
            doc.setFont('helvetica', 'normal');
            doc.text('Total Audit Entries', margin, y);
            doc.setTextColor(13, 13, 13);
            doc.setFont('helvetica', 'bold');
            doc.text(String(c.audit_entries), pageWidth - margin, y, { align: 'right' });
            y += 12;
            doc.line(margin, y, pageWidth - margin, y);
            y += 8;
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text('Generated by SwiftLink — Confidential', margin, 287);
                doc.text(`Page ${i} of ${pageCount}`, pageWidth - margin, 287, { align: 'right' });
            }

            const date = new Date().toISOString().split('T')[0];
            doc.save(`swiftlink-compliance-${date}.pdf`);
        } catch (err) {
