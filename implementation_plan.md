...
### [Sprint 19] — Resolve 32 code quality issues across audit
- **Date:** 2026-03-28
- **Files modified:** `frontend/src/pages/manager/ManagerComplaintsPage.jsx`, `frontend/src/pages/manager/ManagerDriversPage.jsx`, `frontend/src/pages/manager/ManagerVehiclesPage.jsx`, `frontend/src/pages/driver/DriverTripsPage.jsx`, `frontend/src/components/DriverTripCard.jsx`, `frontend/src/components/ActiveTripCard.jsx`, `frontend/src/pages/BookingHistoryPage.jsx`, `frontend/src/pages/SwiftlinkHomePage.jsx`, `frontend/src/pages/LoginPage.jsx`, `frontend/src/pages/manager/ManagerDispatchPage.jsx`, `backend/routes/complaints.js`
- **What changed:** Pattern 1 (error field): replaced `data?.message` with `data?.error || data?.message` in LoginPage, ManagerDriversPage, ManagerVehiclesPage, ManagerComplaintsPage, ManagerDispatchPage; Pattern 2 (missing await): added `await` before `fetchTrips()` in DriverTripsPage handleAccept/handleDecline, `await fetchDrivers()` in ManagerDriversPage, `await fetchVehicles()` in ManagerVehiclesPage, `await fetchComplaints()` in ManagerComplaintsPage; Pattern 3 (loading states): added `formLoading` to ManagerVehiclesPage wrapping add/delete handlers, `actionLoading` to ManagerComplaintsPage wrapping status/notify handlers, `accepting` state to DriverTripCard Accept button, `completing` prop to ActiveTripCard Confirm button; Pattern 4 (double-submit): disabled buttons during in-flight requests across ManagerComplaintsPage (selects + Notify button), ManagerVehiclesPage (Add/Remove buttons), DriverTripCard (Accept button), ActiveTripCard (Confirm button); Pattern 5 (error display): added `error` state + render in BookingHistoryPage, `contactError` state + inline message in SwiftlinkHomePage contact form; Pattern 6 (backend validation): added `notes` field validation (400 if missing or not a non-empty string) on PATCH `/:complaintId/notes` in complaints.js
- **Why:** Audit identified error-field mismatches causing silent failures, missing awaits causing stale UI after mutations, absent loading guards enabling double-submission, and unhandled catch paths leaving users without feedback

### [Sprint 19] — Switch email provider to Brevo HTTP API
- **Date:** 2026-03-28
...
- **What changed:** Complete rewrite of BookingLandingPage with three load states controlled by a `view` state variable (`'loading' | 'booking-form' | 'history' | 'trip'`); `BookingFormView` component added — full booking form with all fields including `additional_info` ("Additional notes for your driver"), handles 409 concurrent booking with inline recovery CTA, shows "check inbox" success state, has "already have a booking?" footer recovery; `HistoryView` component added — fetches `GET /bookings/history`, shows past trips with status pills, "Book a New Ride" button, "Remove this app" option (clears cookie via `POST /bookings/logout` then shows PWA removal instructions); trip interface updated with: cancel button at `pending`/`accepted` with inline confirm (sets status to `cancelled`), ETA badge shown at `accepted` when `booking.eta` is set (formatted in Africa/Nairobi), driver card now shows vehicle make/model/plate via `vehicleDisplay` helper, `additional_info` read-only display at all statuses with inline edit button at `accepted` (modal shows only that field), full edit modal at `pending` includes `additional_info` field, `cancelled` status badge added; `POST /bookings/logout` endpoint added to backend to clear HttpOnly cookie on demand; `isCancelled` state shows compact cancelled banner with "Book new →" CTA; useRef guard prevents double-fire on auth init
- **Why:** Clients need a complete self-service interface — the three-state model covers every session scenario (new, returning, history-only) without requiring accounts; cancel gives clients agency before the driver departs; ETA and vehicle details reduce anxiety; additional_info editable at accepted allows last-minute driver briefing

### [Sprint 20] — Batch 4–5: WebSocket reconnection and logout
- **Date:** 2026-05-12
- **Files modified:** `frontend/src/hooks/useChat.js`, `backend/routes/bookings.js`
- **What changed:** Socket.IO reconnection config added — exponential backoff 1s→30s, Infinity attempts, 0.5 jitter factor; `connect_error` handler changed from `setError` (showed banner on every retry) to silent `console.warn` (permanent rejections still surface via `auth_error` event); `POST /bookings/logout` added to backend
- **Why:** Mobile PWA on Kenyan networks loses connectivity frequently — suppressing transient `connect_error` banners prevents repeated false-alarm UX while still surfacing genuine auth failures
...
- **What changed:** Added `additional_info TEXT` (nullable) and `eta TIMESTAMPTZ` (nullable) columns to `trips` table; added `make TEXT` and `model TEXT` (nullable) columns to `vehicles` table; `PATCH /api/trips/:tripId/assign` now accepts optional `eta` and writes it in the assignment transaction; `POST /api/bookings` accepts and stores `additional_info`; `GET /api/bookings/:tripId` now returns `additional_info`, `eta`, `vehicle_make`, `vehicle_model`, `vehicle_plate` for client-facing display; driver trip queries (`GET /` and `GET /:tripId` in driverTrips) now include `additional_info` and `eta`; ETA decision: manager sets it on assignment as a `TIMESTAMPTZ`
- **Why:** Data foundation required before any downstream Batch 2–8 work — all client PWA features, driver trip notes display, and ETA visibility depend on these fields existing in the schema

### [Sprint 20] — Batch 8: Client push notifications on driver acceptance
- **Date:** 2026-05-12
- **Files modified:** `backend/database/migrations/006_add_client_push_subscriptions.sql`, `backend/database/schema.sql`, `backend/utils/sendClientPushNotification.js`, `backend/routes/bookings.js`, `backend/routes/driverTrips.js`, `frontend/src/pages/BookingLandingPage.jsx`
- **What changed:** New `client_push_subscriptions` table (email-keyed, not UUID, because clients have no persistent account) with unique index on endpoint and btree index on client_email; `sendClientPushNotification(email, payload)` utility mirrors sendPushNotification pattern — VAPID at call time, cleans stale 404/410 endpoints, no notification history record (no client_notifications table needed); `POST /bookings/push-subscribe` and `DELETE /bookings/push-subscribe` endpoints added behind `requireClientAuth` (HttpOnly cookie), upsert on endpoint handles key rotation; driver accept endpoint (`PATCH /driverTrips/:tripId/accept`) now fire-and-forgets a push to the client after setting status `in_progress` — payload includes driver first name and vehicle make/model/plate (never contact details); push permission prompt added to trip view in BookingLandingPage — shows only when `Notification.permission === 'default'`, on enable fetches VAPID key, subscribes via PushManager, then POSTs to `/bookings/push-subscribe`; complaint email on status change was already wired in complaints.js (no change needed)
- **Why:** Driver acceptance is the highest-anxiety moment for a waiting client; a push at that moment reduces check-in polling and closes the loop without giving the driver the client's phone number; permission is requested at the booking confirmation moment (trip view) not on app load — avoids cold-prompt rejection and aligns with when the user most cares about being notified

### [Sprint 20] — Batch 9: Offline resilience, timezone handling, performance audit
- **Date:** 2026-05-12
- **Files modified:** `frontend/src/hooks/useOnlineStatus.js` (new), `frontend/src/styles/animations.css`, `frontend/src/components/BookingCard.jsx`, `frontend/src/components/DriverTripCard.jsx`, `frontend/src/pages/BookingLandingPage.jsx`, `frontend/src/components/ChatWindow.jsx`
- **What changed:** `useOnlineStatus` hook — listens to window online/offline events, returns boolean; `animations.css` — `@media (prefers-reduced-motion: reduce)` block disables all decorative float/morph/drift/spin animations and removes stagger delays, preserves functional session-pulse and shimmer indicators; `BookingCard` and `DriverTripCard` — all pickup_time displays now use `timeZone: Africa/Nairobi` with EAT label (was using browser locale with no TZ); `BookingLandingPage` — imports `useOnlineStatus`, shows subtle red "offline" pill in sticky nav when disconnected, `handleComplaintSubmit` saves to localStorage when offline and sets `queuedComplaint` state, `useEffect` drains queued complaint automatically when connectivity restores, `queuedComplaint` state shows a "Complaint Queued" card with the category label, complaint submit button changes label to "Queue Complaint" when offline, pickup time in trip details card fixed to Africa/Nairobi + EAT label; `ChatWindow` — pre-connection screen now only shows when `messages.length === 0` (was showing on every reconnect, destroying message history), header dot dims and shows "reconnecting…" text while disconnected, `pendingMessages` state queues messages typed while offline, pending messages are shown at 55% opacity with ⏳ timestamp indicator, flushed to socket on reconnect, send button and textarea both work while offline (was disabled)
- **Why:** Client PWA runs on Kenyan mobile networks — brief connectivity drops are common; complaint queuing ensures the 24-hour window is usable even on poor connectivity; reduced motion removes heavy CSS animations on low-end Android where GPU compositing is expensive; timezone label prevents confusion since UTC stored times would display incorrectly for EAT users

### [Sprint 21] — Batch 1: Manager in-system help page
- **Date:** 2026-05-13
- **Files modified:** `frontend/src/pages/manager/ManagerHelpPage.jsx` (new), `frontend/src/App.jsx`, `frontend/src/components/layout/ManagerLayout.jsx`
- **What changed:** New `/manager/help` page — 7 collapsible glass-card sections (Getting Access, Creating a Booking, Assigning Driver, Monitoring Trips, Privacy Dashboard, Complaints, Audit Export); accordion expand/collapse with one-open-at-a-time; step circles in #6C63FF, privacy notes in purple tint, caution notes in amber tint, data tables with `glass-card` styling; `?` NavLink added to desktop pill-nav right actions, mobile sticky header (right of user name), and tablet drawer nav
- **Why:** Lecturer feedback — help content must be findable inside the system, not only in the Word documentation; dedicated page chosen for manager because they are desktop users who need to read at their own pace and refer back during multi-step workflows

### [Sprint 21] — Batch 2: Driver in-system help sheet
- **Date:** 2026-05-13
- **Files modified:** `frontend/src/components/DriverHelpSheet.jsx` (new), `frontend/src/components/layout/DriverLayout.jsx`
- **What changed:** New `DriverHelpSheet` bottom sheet component — slides up from any driver page via a `?` button; `deriveContext(pathname)` switches content across 4 contexts: trips-list (status badge legend, accept steps, decline steps), trip-detail (chat channel guide, complete trip steps), profile (push notification setup and browser permission recovery), notifications (notification history explainer); body scroll locked while open; backdrop + slide-up transition; `?` button added to mobile sticky header (right of driver name), desktop/tablet pill-nav right group, and tablet drawer
- **Why:** Drivers are mobile and mid-task — a bottom sheet is the correct pattern for contextual help without leaving the current page; route-aware content means the help shown is always relevant to what the driver is currently doing

### [Sprint 21] — Batch 3: Client in-system help modal
- **Date:** 2026-05-13
- **Files modified:** `frontend/src/components/ClientHelpModal.jsx` (new), `frontend/src/pages/BookingLandingPage.jsx`, `frontend/src/pages/BookingHistoryPage.jsx`
- **What changed:** New `ClientHelpModal` — self-contained component (manages own open/close state) with a fixed floating `?` button at `bottom: 88px, right: 20px` and a bottom sheet overlay; content switches across 6 contexts: booking-form (access link explainer, email usage, privacy note), trip-pending (awaiting assignment guidance), trip-accepted (driver details, chat steps, cancel steps), trip-active (chat guide, complaint window, privacy data lifecycle), trip-ended (post-trip complaint window, cancellation), history (read-only session explainer, missing data rationale); added to `BookingFormView` (context="booking-form"), `HistoryView` (context="history"), and main trip view (helpContext derived from booking.status); also added to `BookingHistoryPage` (context="history")
- **Why:** Clients have no persistent account and no other navigation — a floating button is the only entry point that works across all three load states of BookingLandingPage without interfering with content; context-switching means the help is always relevant to the client's current situation

### [Sprint 21] — Batch 4: Corporate enquiries feature
- **Date:** 2026-05-13
...
### [Sprint 24] — AviationStack flight info integration
- **Date:** 2026-05-14
- **Files modified:** `backend/routes/flights.js` (new), `backend/routes/index.js`, `frontend/src/hooks/useFlightInfo.js` (new), `frontend/src/pages/driver/DriverActiveTripPage.jsx`, `frontend/src/components/BookingCard.jsx`
- **What changed:** Backend — new `GET /api/flights/info?iata=&date=` endpoint behind `requireAuth`; fetches from AviationStack HTTP API; Redis-caches results with smart TTL (landed/cancelled: 3600s, airborne: 180s, scheduled <2h: 300s, scheduled >2h: 1800s, no result: 300s); normalises IATA codes (strips spaces, uppercase); mounted in `index.js`. Frontend — `useFlightInfo` hook: accepts `flightNumber`, `date`, and `eager` params; `eager=true` auto-fetches on mount, `eager=false` exposes `load()` for on-demand trigger; `DriverActiveTripPage` — replaces static "TRACKING" badge with live status badge (Scheduled/Airborne/Landed/Cancelled/Diverted colour-coded), expandable details panel showing origin airport, scheduled/estimated/actual arrival in EAT, delay minutes, terminal/gate, refresh button; auto-loads when trip has a flight_number; `BookingCard` — flight IATA badge is now a "Check →" button (click-to-reveal to save API quota); inline details panel expands below badge on first click
- **Why:** Airport transfers are a core use case for Kenyan fleet operators; arrival time determines driver pickup window; free-tier quota is limited (~100 req/month) so manager cards are click-to-reveal (demand-only) and the driver auto-loads once per trip session (backed by Redis cache, so second render is free)

### [Sprint 23] — Fix driver notes display + client→manager contact
- **Date:** 2026-05-14
- **Files modified:** `backend/routes/driverTrips.js`, `backend/routes/bookings.js`, `frontend/src/components/DriverTripCard.jsx`, `frontend/src/pages/driver/DriverActiveTripPage.jsx`, `frontend/src/pages/BookingLandingPage.jsx`
- **What changed:** Backend — added `t.notes`, `t.flight_number` to both driver trip SELECT queries (GET / and GET /:tripId); added `POST /bookings/contact-manager` endpoint (requireClientAuth, sends email to manager via `CONTACT_ENQUIRY_EMAIL`). Frontend — fixed `trip.registration_number` → `trip.vehicle_reg` in DriverTripCard; replaced phantom `trip.special_requirements` with `trip.notes` (Manager Instructions) and `trip.additional_info` (Client Notes) in DriverActiveTripPage Manifest section; added "Message Fleet Manager" section to BookingLandingPage trip view (non-cancelled states) with textarea, send handler, sent/error feedback
- **Why:** Driver could never see manager instructions or client notes (field name mismatch + missing fields in SQL query); vehicle registration never rendered in trip card (wrong alias); client had no way to contact the manager from the booking page

### [Sprint 25] — Driver↔Manager DMs, client flight info, dispatch enhancements
- **Date:** 2026-05-14
- **Files modified:** `backend/database/migrations/008_direct_messages.sql` (new), `backend/routes/driverTrips.js`, `backend/routes/trips.js`, `frontend/src/pages/driver/DriverActiveTripPage.jsx`, `frontend/src/components/BookingCard.jsx`, `frontend/src/pages/manager/ManagerDispatchPage.jsx`, `frontend/src/pages/BookingLandingPage.jsx`
- **What changed:**
  - DB: new `direct_messages` table (trip_id, sender_role CHECK IN ('driver','fleet_manager'), body, created_at); indexed on (trip_id, created_at)
  - Backend driverTrips: added `GET /:tripId/direct-messages` and `POST /:tripId/direct-message` (gated on in_progress); driver DM fires `emitDashboardEvent('direct_message', ...)`
  - Backend trips: added `GET /:tripId/direct-messages` and `POST /:tripId/direct-message` for fleet_manager; manager DM creates driver_notification entry + fires driver push
  - DriverActiveTripPage: added "Manager Channel" DM section (only rendered when in_progress); polls GET every 10s; send gated by in_progress; 24hr EAT timestamps on all flight times; `formatEATDate` added
  - BookingCard: `formatEAT` updated to `hour12: false`; flight panel now shows departure airport + dep time + divider + arrival airport + arr time + delay + terminal/gate; uses `formatEATDate` for full date+time
  - ManagerDispatchPage: `dmTripId`/`tripDms`/`dmInput`/`sendingDm` state; `useRef` for `dmTripId` inside socket closure; socket listener for `direct_message` event; `useEffect` polls DMs when panel open; slide-in DM panel with message history + reply input; "DM" button on active trip mini-cards; `client_corporate_email` shown alongside driver name in active + awaiting sections; `additional_info` field added to new booking form; booking payload includes `additional_info`
  - BookingLandingPage: contact manager form auto-resets to idle after 3s (setTimeout); `flightInfo`/`flightLoading`/`flightError`/`flightLoaded` state; auto-fetches `/bookings/flight-info` once when booking has a flight_number (client auth, reuses Redis cache); full flight panel (dep/arr airports, times, delay, terminal/gate, refresh) shown in trip view
- **Why:** All three roles now communicate within the system — no informal channels needed; flight data surfaces to all parties from a single Redis-cached API call; manager can see client identity on all trip cards; booking form allows pre-populating client notes

### [Sprint 26] — Always-on messages, manager-issued password reset, demo seed, confinement-focused PDF
- **Date:** 2026-05-15
- **Files modified:**
  - **DB:** `backend/database/migrations/009_global_direct_messages.sql` (new), `backend/database/migrations/010_driver_password_resets.sql` (new)
  - **Backend:** `backend/routes/messages.js` (new), `backend/routes/index.js`, `backend/routes/roster.js`, `backend/routes/drivers.js`, `backend/routes/complaints.js`, `backend/routes/dashboard.js`, `backend/routes/flights.js`, `backend/routes/bookings.js`, `backend/config/mailer.js`, `backend/database/seed.js`
  - **Frontend:** `frontend/src/pages/manager/ManagerMessagesPage.jsx` (new), `frontend/src/pages/driver/DriverManagerPage.jsx` (new), `frontend/src/pages/DriverResetPasswordPage.jsx` (new), `frontend/src/components/ClientManagerThread.jsx` (new), `frontend/src/App.jsx`, `frontend/src/components/layout/ManagerLayout.jsx`, `frontend/src/components/layout/DriverLayout.jsx`, `frontend/src/pages/manager/ManagerDriversPage.jsx`, `frontend/src/pages/manager/ManagerAuditPage.jsx`, `frontend/src/pages/ManagerPrivacyDashboardPage.jsx`, `frontend/src/pages/BookingLandingPage.jsx`, `frontend/src/pages/driver/DriverNotificationsPage.jsx`, `frontend/src/utils/compliancePdf.js`
- **What changed:**
  - DB: migration 009 makes `direct_messages.trip_id` nullable and adds `driver_id` + `client_email` so DMs can be always-on (not trip-scoped); CHECK constraint enforces exactly one of (driver_id, client_email, trip_id); read-state columns `read_by_manager_at`/`read_by_recipient_at`. Migration 010 adds `driver_password_resets` (token, expires_at, used_at) and idempotently ensures `complaints.investigation_notes`.
  - Backend messages routes: GET/POST `/api/messages/threads/drivers|clients` (manager), `/api/messages/threads/driver/:id`, `/api/messages/threads/client/:email`, `/api/messages/driver/mine` (driver), `/api/messages/client/mine` (client cookie). Each send emits `direct_message` on the dashboard socket and fires the appropriate web push.
  - Manager-issued driver password reset: `POST /roster/drivers/:id/reset-password` generates a 1h single-use token, emails the driver a branded reset link (`sendDriverPasswordReset` in mailer); `GET /api/drivers/auth/reset/:token` validates; `POST /api/drivers/auth/reset/:token` sets a new bcrypt hash. Replaces the previous "copy/paste temp password" flow.
  - Complaint investigation notes now create a `driver_notifications` row with the manager's note body verbatim and fire a push notification of type `investigation_note`. The driver notifications page renders these expandable in place (read-in-place, not navigation), and direct-message notifications are likewise expandable.
  - Dashboard compliance-report: dropped `regulatory_basis: 'Kenya Data Protection Act 2019…'` field; rewrote `architecture_note` and executive summary to focus on data confinement (no driver-side client identifiers, ephemeral sessions, conditional persistence on complaint). `legal_basis` retained on audit_log rows for existing test coverage.
  - Flight API: better error responses, EAT-formatted (`Africa/Nairobi`) `scheduled_eat`/`estimated_eat`/`actual_eat` fields on both `GET /api/flights/info` and `GET /api/bookings/flight-info`; "no flight found" payload now carries `searched` and `message` fields for clearer UI feedback.
  - Seed: expanded to 25 trips (3 pending, 4 accepted, 3 in_progress, 15 completed), 4 complaints across all states (open / 2 under_investigation with full investigation notes / 1 resolved), 12 enquiries (mix of statuses), 5 driver↔manager + 6 client↔manager DM threads with realistic exchanges, 2 driver notifications with manager investigation notes, audit_log entries for TRIP_ASSIGNED/TRIP_ACCEPTED/TRIP_COMPLETED on every completed trip so the privacy dashboard's compliance-report numbers tell the full confinement story.
  - Frontend: new `/manager/messages` page with Drivers/Clients tabs and unread badges, live-updates via dashboard socket. New `/driver/manager` tab — driver can DM the fleet manager anytime, regardless of trip status. `ClientManagerThread` component replaces the one-shot "Message Fleet Manager" email form on `BookingLandingPage` with a persistent threaded chat. ManagerDriversPage gains a "Reset password" action that triggers the email link. New `/driver/reset-password/:token` page validates the token, accepts a new password, and routes to login. DriverNotificationsPage handles `investigation_note` and `direct_message` types with type-specific icons and an expandable note body. ManagerAuditPage shows a proper error+retry state for enquiries instead of an indefinite "Loading…". `compliancePdf.js` rewritten — removed the DPA Section 25 regulatory block and refocused on a "client identifier confinement: 100%" headline + a how-it-is-enforced narrative.
- **Why:** Driver/client both needed a back-channel to the manager outside an active trip (the previous in-trip chat tied messaging to lifecycle); the seed had to support a credible demo of the confinement angle with realistic complaint and investigation data; the demo storyline argues data confinement, not DPA compliance, so the PDF/UI wording was refocused; "copy this temp password" was a poor onboarding experience compared to a single-use emailed link.
